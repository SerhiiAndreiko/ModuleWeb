...
import sorter
import json
from classes import AddressBook, Name, Phone, Record, Birthday, Address, Email, Note, NoteBook, ConsoleInterface, UserInterface, WebInterface
...
def sort_directory():
    ...
    result = sorter.sort_folder(folder_path)
    return result
...
def main(interface: UserInterface = None):
    ...
    while True:
        ...
        if closest_command:
            user_interface.display_message(f"Did you mean '{closest_command}'")
        else:
            if command in commands:
                func = commands[command]
                output = func()
                user_interface.display_message(output)
            else:
